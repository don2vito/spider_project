...
class XHSClient:
    ...
    def get_video_url(self, note: dict) -> str:
        """Extract the best video download URL from note data.
        Tries multiple paths:
          1. video.media.stream.h264/h265 (multi-bitrate, pick highest resolution)
          2. video.consumer.originVideoKey (original video key)
          3. video.originVideoKey / video.origin_video_key (direct key)
        Returns empty string if no video URL found.
        """
        video = note.get("video") or {}
        if not isinstance(video, dict) or not video:
            return ""

        # Path 1: video.media.stream.h264/h265 (multi-bitrate streams)
            ...
            if isinstance(stream, dict):
                ...
                for codec in ["h265", "h264"]:
                    ...
                    for s in streams:
                        ...
                        # Try backupUrls first (more reliable), then masterUrl
                        urls = s.get("backupUrls") or []
                        master = s.get("masterUrl") or ""
                        url = ""
                        if isinstance(urls, list) and urls:
                            url = urls[0] if isinstance(urls[0], str) else ""
                        if not url and master:
                            url = master
        ...
        # Path 2: video.consumer.originVideoKey
        consumer = video.get("consumer") or {}
        if isinstance(consumer, dict):
            key = consumer.get("originVideoKey") or consumer.get("origin_video_key") or ""
            if key:
                return f"http://sns-video-bd.xhscdn.com/{key}"

        # Path 3: video.originVideoKey / video.origin_video_key (direct)
        key = video.get("originVideoKey") or video.get("origin_video_key") or ""
        if key:
            return f"http://sns-video-bd.xhscdn.com/{key}"
...
class ZIPExporter:
    ...
    async def export_note(self, note: dict, comments: list = None) -> BytesIO:
        ...
        with zipfile.ZipFile(buf, 'w', zipfile.ZIP_DEFLATED) as zf:
            ...
            # 4. Video
            video_url = self.client.get_video_url(note)
            if video_url:
                print(f"[Export] Downloading video: {video_url[:80]}...")
                try:
                    # Remove backtick prefix if present
                    ...
                    video_data = await self.client.download_file(video_url, timeout=120.0)
                    if video_data and len(video_data) > 1000:  # Sanity check: video should be > 1KB
                        zf.writestr(f"{safe_title}/video.mp4", video_data)
                        print(f"[Export] Video downloaded: {len(video_data)} bytes")
                    else:
                        print(f"[Export] Video data too small ({len(video_data)} bytes), skipping")
                except Exception as e:
                ...
                if video_url:
                    try:
                        ...
                        if video_data and len(video_data) > 1000:
                            zf.writestr(f"{safe_title}/video.mp4", video_data)
                            print(f"[Export] Video downloaded: {safe_title} ({len(video_data)} bytes)")
                    except Exception as e:
                        print(f"[Export] Video download failed for {safe_title}: {e}")
